...
def test_simple_spline_plot():
    ...
    fish_data = FishPlotData(frac_table=FRAC_TABLE,
                             parents=PARENTS,
...
@pytest.mark.mpl_image_compare(baseline_dir='baseline_images/test_plot',
                               ...
                               tolerance=15)
def test_simple_plot_with_annotations_outline(): # Renamed function
    """
...
def test_panel_b_polygon_shape():
    ...
    """
    fish_data = FishPlotData(frac_table=FRAC_TABLE_B,
                             parents=PARENTS_B,
...
def test_panel_c_separate_clones():
    ...
    """
    fish_data = FishPlotData(frac_table=FRAC_TABLE_C,
                             parents=PARENTS_C,
...
def test_custom_legend_spacing():
    ...
    fish_data = FishPlotData(frac_table=FRAC_TABLE,
                             parents=PARENTS,
                             timepoints=TIMEPOINTS,
                             clone_labels=CLONE_LABELS)  # Use defined labels
    fish_data.layout_clones()
...
def test_fig2b_annotation_style():
    ...
    fish_data = FishPlotData(frac_table=FRAC_TABLE_F2B,
                             parents=PARENTS_F2B,
    ...
    # Add panel label 'A' as in R test? (R's test plots A/B differently)
    # The R test calls this plot 'A' in the layout, but uses Fig2B data/style.
    # Let's skip the panel label for simplicity in Python test.
